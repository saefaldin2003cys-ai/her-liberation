...
function shareArticle(articleId) {
    ...
    // Translations
    var shareTitle = lang === 'en' ? 'Share Article' : 'مشاركة المقال';
    var copyLinkText = lang === 'en' ? 'Copy Link' : 'نسخ الرابط';
    var copiedText = lang === 'en' ? 'Copied!' : 'تم النسخ!';

    var modal = document.getElementById('detailsModal');
    var modalBody = document.getElementById('modalBody');

    modalBody.innerHTML = '<div class="share-modal-content">' +
        '<h3 class="modal-title">📤 ' + shareTitle + '</h3>' +
        '<p class="share-article-title">' + escapeHTML(title) + '</p>' +
        '<div class="share-platforms">' +
        '<button class="share-platform-btn whatsapp" onclick="shareToWhatsApp(\'' + encodeURIComponent(text) + '\')">' +
        ...
        '<span>WhatsApp</span>' +
        '</button>' +
        '<button class="share-platform-btn twitter" onclick="shareToTwitter(\'' + encodeURIComponent(text) + '\')">' +
        '<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>' +
        '<span>X</span>' +
        '</button>' +
        '<button class="share-platform-btn facebook" onclick="shareToFacebook(\'' + encodeURIComponent(url) + '\')">' +
        '<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>' +
        '<span>Facebook</span>' +
        '</button>' +
        '<button class="share-platform-btn telegram" onclick="shareToTelegram(\'' + encodeURIComponent(text) + '\')">' +
        ...
        '<span>Telegram</span>' +
        '</button>' +
        '<button class="share-platform-btn copy-link" onclick="copyArticleLink(\'' + url + '\', this)">' +
        ...
        '</button>' +
        '</div>' +
        '<button class="cancel-btn" onclick="closeModal()" style="margin-top: 1rem;">' + (lang === 'en' ? 'Close' : 'إغلاق') + '</button>' +
        '</div>';
...
function copyArticleLink(url, button) {
    navigator.clipboard.writeText(url).then(function () {
        ...
        var lang = window.i18n ? window.i18n.getCurrentLanguage() : 'ar';
        button.querySelector('span').textContent = lang === 'en' ? 'Copied!' : 'تم النسخ!';
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updatePollResults(data) {
        // Calculate total from only agree18 and disagree (not including other)
        var total = (data.agree18 || 0) + (data.disagree || 0);
        if (total === 0) total = 1; // Prevent division by zero

        var agreePercent = Math.min(100, Math.round((data.agree18 / total) * 100));
        var disagreePercent = Math.min(100, Math.round((data.disagree / total) * 100));
